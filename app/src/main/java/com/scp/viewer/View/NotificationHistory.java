/*
  ClassName: NotificationHistory.java
  AppName: ViewerApp
  Created by Lucas Walker (lucas.walker@jexpa.com)
  Created Date: 2021-07-19
  Description: The NotificationHistory class is used to display the history viewed by the target user on the notification
...
import com.scp.viewer.Adapter.AdapterNotificationHistory;
import com.scp.viewer.Database.DatabaseLastUpdate;
import com.scp.viewer.Database.DatabaseNotification;
...
public class NotificationHistory extends AppCompatActivity {
    ...
    List<Notifications> mData = new ArrayList<>();
    List<Notifications> notificationsListAdd = new ArrayList<>();
    // action mode
    public static boolean isInActionMode;
    public static ArrayList<Notifications> selectionList;
    private DatabaseNotification database_Notification;
    private SwipeRefreshLayout swp_Notification_History;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        database_Notification = new DatabaseNotification(this);
        database_last_update = new DatabaseLastUpdate(this);
        table = (Table) getIntent().getSerializableExtra(TABLE_NOTIFICATION_HISTORY);
        // show dialog Loading...
        getNotificationInfo();
        swipeRefreshLayout();
    }
    ...
    private void setID() {
        toolbar = findViewById(R.id.toolbar_Notification_History);
        toolbar.setTitle(MyApplication.getResourcses().getString(R.string.NOTIFICATION_HISTORY));
        ...
        txt_No_Data_Notification = findViewById(R.id.txt_No_Data_Notification);
        progressBar_Notification = findViewById(R.id.progressBar_Notification);
        progressBar_Notification.setVisibility(View.GONE);
        swp_Notification_History = findViewById(R.id.swp_NotificationHistory);
        avLoadingIndicatorView = findViewById(R.id.aviNotification);
        // recyclerView
        mRecyclerView = findViewById(R.id.rcl_Notification_History);
    ...
    private void getNotificationInfo() {
        ...
        if (isConnected(this)) {
            ...
            int i = database_Notification.get_Notifications_Count_DeviceID(table.getID());
            if (i == 0) {
                txt_No_Data_Notification.setVisibility(View.VISIBLE);
                txt_No_Data_Notification.setText(MyApplication.getResourcses().getString(R.string.NoData));
                txt_Total_Data.setText("0");
            } else {
                mData.clear();
                mData = database_Notification.getAll_Notification_ID_History(table.getID(),0);
                mAdapter = new AdapterNotificationHistory(this, (ArrayList<Notifications>) mData);
                ...
                txt_No_Data_Notification.setText(MyApplication.getResourcses().getString(R.string.TurnOn));
                txt_Total_Data.setText(getSharedPreferLong(getApplicationContext(), NOTIFICATION_TOTAL + table.getDevice_Identifier())+"");
            }
        }
    }
                ...
                if (isConnected(getApplicationContext()))
                {
                    if ((calendar.getTimeInMillis() - time_Refresh_Device) > LIMIT_REFRESH) {
                        ...
                        new Handler().postDelayed(new Runnable() {
                            ...
                            public void run()
                            {
                                swp_Notification_History.setRefreshing(false);
                    ...
                    swp_Notification_History.setRefreshing(false);
                    noInternet(NotificationHistory.this);
                }
            }
        });
    }
            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run()
                {
                    ...
                    if(isConnected(getApplicationContext()))
                    {
                        ...
                        long totalContact = getSharedPreferLong(getApplicationContext(), NOTIFICATION_TOTAL + table.getDevice_Identifier());
                        new getNotificationAsyncTask(currentSize+1).execute();
                        ...
                        List<Notifications> mDataCall = database_Notification.getAll_Notification_ID_History(table.getID(),currentSize);
                        // Here is the total item value contact of device current has on Cpanel
    ...
    private class getNotificationAsyncTask extends AsyncTask<String, Void, String>
    {
        ...
        public getNotificationAsyncTask(long startIndex) {
        ...
        protected String doInBackground(String... strings) {
            ...
            return GetJsonFeature(table, this.startIndex, GET_NOTIFICATION_HISTORY);
        }
            ...
            try {
                ...
                if (NotificationJson.length() != 0) {
                    ...
                    for (int i = 0; i < NotificationJson.length(); i++) {
                        ...
                        Gson gson = new Gson();
                        Notifications notifications = gson.fromJson(String.valueOf(NotificationJson.get(i)), Notifications.class);
                        notifications.setNotification_Title(notifications.getNotification_Title().replace("/","").replace("=",""));
                        notificationsListAdd.add(notifications);
                        Log.d("NotificationHistory"," Add Notification = "+  notifications.getNotification_Title());

                    }
                    if (notificationsListAdd.size() != 0) {
                        database_Notification.addNotification(notificationsListAdd);
                    }
                }
                //mData.clear();
                Log.d("NotificationHistory"," currentSize Notification = "+  currentSize+ " checkLoadMore = "+ checkLoadMore);
                List<Notifications> mDataTamp = database_Notification.getAll_Notification_ID_History(table.getID(),currentSize);
                ...
                if(checkLoadMore)
                {
                    ...
                    mData.addAll(insertIndex, mDataTamp);
                    Log.d("checkdata"," MData Notification = "+ mDataTamp.size());
                    ...
                    mAdapter = new AdapterNotificationHistory(NotificationHistory.this, (ArrayList<Notifications>) mData);
                    mRecyclerView.setAdapter(mAdapter);
                    mAdapter.notifyDataSetChanged();
                }
                lnl_Total.setVisibility(View.VISIBLE);
                if (mData.size() == 0) {
                    ...
                    database_last_update.update_Last_Time_Get_Update(TABLE_LAST_UPDATE, COLUMN_LAST_NOTIFICATION, max_Date, table.getDevice_Identifier());
                    String min_Time1 = database_last_update.getLast_Time_Update(COLUMN_LAST_NOTIFICATION, TABLE_LAST_UPDATE, table.getDevice_Identifier());
                    Log.d("min_time1", min_Time1 + "");
                    txt_No_Data_Notification.setText(("Last update: "+ APIDatabase.getTimeItem(database_last_update.getLast_Time_Update(COLUMN_LAST_NOTIFICATION, TABLE_LAST_UPDATE, table.getDevice_Identifier()),null)));
                    txt_Total_Data.setText(getSharedPreferLong(getApplicationContext(), NOTIFICATION_TOTAL + table.getDevice_Identifier())+"");
                }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.item_delete) {
            if (isConnected(NotificationHistory.this)) {

                alertDialogDeleteItems(NotificationHistory.this,
    ...
    private class clear_App extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
           ...
           /* String value = "<RequestParams Device_ID=\"" + table.getDevice_Identifier() + "\" List_ID=\"" + listID + "\" />";
            ...
            return PostJsonClearDataToServer(table.getDevice_Identifier(), listID, POST_CLEAR_MULTI_NOTIFICATION);
        }
        ...
        protected void onPostExecute(String s) {
            ...
            if (bodyLogin.getResultId().equals("1") && bodyLogin.getIsSuccess().equals("1")) {
                ((AdapterNotificationHistory) mAdapter).removeData(selectionList);
                clearDataSQLite(selectionList);
                clearActionMode();
            } else {
                clearActionMode();
                Toast.makeText(NotificationHistory.this, bodyLogin.getDescription(), Toast.LENGTH_SHORT).show();
            }
    ...
    // delete on SQLite
    public void clearDataSQLite(ArrayList<Notifications> selectionList) {
        for (Notifications notifications : selectionList) {
            database_Notification.delete_Notifications_History(notifications);
        }
    }
